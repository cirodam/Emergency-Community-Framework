...
/** Derived: current bank permission levels (empty array when not logged in). */
const bankPerms = derived(session, s => s?.appPermissions?.["bank"] ?? []);
export const isTeller    = derived(bankPerms, p => p.includes("teller") || p.includes("admin"));
export const isBankAdmin = derived(bankPerms, p => p.includes("admin"));

/** Derived: which page to show — simple string-based router */
export type Page = "login" | "accounts" | "account" | "send" | "history" | "settings" | "admin";
